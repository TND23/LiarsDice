...
def train_deep_q(agent: DeepQLearningAgent, num_episodes: int, q_table_name=CUR_Q_TABLE_NAME) -> Tuple[List[float], List[float]]:
...
def evaluate_against_agentagent (DeepQLearningAgent, opponent: DeepQLearningAgent = None, num_games: int = 2) -> float:
    pass
...
def main():
    ...
    total_rewards, win_rates = train_deep_q(agent, num_episodes=50)
